        ...
        {visibles.map((item, idx) => {
          ...
          return (
            <div key={idx} style={{ display: 'grid', gridTemplateColumns: '60px 1fr 80px', gap: 8, padding: '6px 16px', borderBottom: '1px solid #F3F4F6', alignItems: 'start', fontSize: 12 }}>
              ...
              <div>
                <div style={{ color: '#374151', lineHeight: 1.4 }}>{item.descripcion?.slice(0, 120) || '—'}</div>
                {item.evidencia && item.evidencia !== 'No encontrado en el documento' && (
                  <div style={{ color: '#6B7280', fontSize: 11, marginTop: 2, fontStyle: 'italic' }}>"{item.evidencia?.slice(0, 80)}"</div>
                )}
              </div>
...
function _itemsRubricaCompleta(rubricasDetalle, clave, fallback = []) {
  ...
  if (items.length > 0) {
    return items.map(i => ({
      ...i,
      estado: i.estado === 'observado' ? 'observado' : (i.estado || 'observado'),
...
function VeredictoCard({ respuesta, error, latencia_ms, agentes, rubricaIds, rubricasDetalle, disenoInfo }) {
  if (error) return (
  ...
  const esRechazado    = textoUp.includes('RECHAZ') || textoUp.includes('NO APROBADO')
  const esObservaciones = !esRechazado && textoUp.includes('OBSERV')
  ...
  return (
    <div>
      ...
      {tieneSecciones ? (
        ...
        <>
          {itemsMet.length > 0 && <SeccionAgente titulo="Agente Metodológico" icono="📐" color="#1D4ED8" bg="#EFF6FF" texto="" items={itemsMet} />}
          {itemsTec.length > 0 && <SeccionAgente titulo="Agente Técnico"       icono="⚙️" color="#6D28D9" bg="#F5F3FF" texto="" items={itemsTec} />}
          {itemsLin.length > 0 && <SeccionAgente titulo="Agente Lingüístico"   icono="📝" color="#065F46" bg="#ECFDF5" texto="" items={itemsLin} />}
          <div style={{ background: '#F8FAFC', border: '1px solid #E5E7EB', borderRadius: 10, padding: 16 }}>
            ...
            <div style={{ fontSize: 13, color: '#374151', whiteSpace: 'pre-wrap', lineHeight: 1.7, maxHeight: 500, overflowY: 'auto' }}>{respuesta}</div>
          </div>
        </>
      )}
...
function QuickAnalysisCard({ texto, latencia_ms, disenoInfo }) {
  const txt = texto || ''
  const veredicto = _extraerVeredictoSimple(txt)
  const rechazado = /RECHAZ/i.test(veredicto)
  const aprobado = /APROB/i.test(veredicto) && !/OBSERV/i.test(veredicto)
  const color = rechazado ? '#7F1D1D' : aprobado ? '#065F46' : '#92400E'
  const bg = rechazado ? '#FEE2E2' : aprobado ? '#D1FAE5' : '#FEF3C7'
  const icono = rechazado ? '❌' : aprobado ? '✅' : '⚠️'

  const secMet = _extraerSeccion(txt, 'METODOLOGICO') || txt
  const secTec = _extraerSeccion(txt, 'TECNICO') || txt
  const secLin = _extraerSeccion(txt, 'LINGUISTICO') || txt

  const estadoMet = _obtenerEstadoDiagnostico(secMet)
  const estadoTec = _obtenerEstadoDiagnostico(secTec)
  const estadoLin = _obtenerEstadoDiagnostico(secLin)

  let fortalezas = _extraerListaSimple(txt, 'Fortalezas', 3)
  let debilidades = _extraerListaSimple(txt, 'Debilidades', 3)
  let recomendaciones = _extraerListaSimple(txt, 'Recomendaciones', 3)
  if (debilidades.length === 0) debilidades = (txt.match(/\d+\.\s*[^\n]*(?:FALTA|No se|Ausencia|Errores|carece)[^\n]*/gi) || []).slice(0, 3).map(x => x.replace(/^\d+\.\s*/, ''))
          ...
          <div style={{ background: '#fff', borderRadius: 16, padding: 32, boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}>
            ...
            {analyzing ? (
              ...
              <>
                ...
                {disenoInfo && (
                  <div style={{ border: '2px solid #7C3AED', borderRadius: 12, overflow: 'hidden', marginBottom: 16 }}>
                    ...
                    {rubrica && (
                      ...
                      return (
                        <>
                          <QuickAnalysisCard texto={txt} latencia_ms={secuencial.latencia_ms} disenoInfo={disenoInfo} />
                ...
                {tipoAnalisis === 'analisis_completo' && (() => {
                  const textoLing = _extraerSeccion(secuencial?.texto_crudo || '', 'lingu')
